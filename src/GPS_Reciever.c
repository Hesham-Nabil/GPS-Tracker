...
void GPS_Start(double *Distance, double coordinates[300][2], char* buffer, int i){   // A function that parces the data after $GPRMC, and retrieve the data in variables
    ...
    {         
        ...
        //while(UART0_RECIEVE_CHAR()!=RMC(i)){

                               // "TO BE" address of the $ sign of the GPRMC
        ...
        if (sscanf(RMC_PTR, "$GPRMC,%lf,%c,%lf,%c,%lf,%c,%lf,", &time, &status, &latitude, &ns, &longitude, &ew, &knots) == 7)
        {
            if (status != 'A')
            continue;
