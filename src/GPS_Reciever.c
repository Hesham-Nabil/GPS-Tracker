...
{ // A function that parces the data after $GPRMC, and retrieve the data in variables
    while (1)
    {
        ...
        if (sscanf(RMC_PTR, "$GPRMC,%lf,%c,%lf,%c,%lf,%c", &time, &status, &latitude, &ns, &longitude, &ew) == 6)
        {
            if (status != 'A')
                continue;
            coordinates[loop_counter][0] = Todegree(longitude); // longitude should be stored in the 1st column
            coordinates[loop_counter][1] = Todegree(latitude);  // lattitude should be stored in the 2nd column
            loop_counter++;
        }
        else
        {
